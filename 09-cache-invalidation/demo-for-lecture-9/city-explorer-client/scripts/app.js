...
function fetchCityData(event) {
  ...
  $.ajax({
    ...
    if (jqXHR.status === 201) {
      console.log(`New location resource created in database for ${searchQuery}`);
    } else if (jqXHR.status === 200) {
      console.log(`Location data retrieved from database for ${searchQuery}`);
    } else {
      console.log('Something went wrong with cache');
    }
    return data;
  })
    .then(location => {
